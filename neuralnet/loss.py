...
class MSSTFTLoss(Loss):
    ...
    def __init__(self, config):
        ...
        self.apply(self.init_parameters)
        fft_scales = config['fft_scales']
        scales = []
        for s in range(fft_scales[1]):
            scales.append(fft_scales[0] * (2 ** s))
        self.scales = scales
        self.overlap = config['overlap']
        self.windows = nn.ParameterList(nn.Parameter(torch.from_numpy(np.hanning(scale)).float().to(config['device']), requires_grad=False) for scale in self.scales)
